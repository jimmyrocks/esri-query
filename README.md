...
If your service is secured by cookies, pass the cookie back as a `Cookie` request header. `Set-Cookie` is the response header the server sends to the client, so it is not what you pass to `esri-query`.
